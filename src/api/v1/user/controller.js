...
exports.register = async (req, res, next) => {
  try {
    ...
    const msg = {
      ...
      html: verifyAccountTemplate(firstName, lastName, token)
    };
...
exports.emailVerification = async (req, res, next) => {
  try {
    ...
    return res.status(httpStatus.PERMANENT_REDIRECT).redirect(SERVER.website);
  } catch (error) {
    return next(error);
  }
};
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    const msg = {
      ...
      html: resetPasswordTemplate(user.firstName, user.lastName, token)
    };
