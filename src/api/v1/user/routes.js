...
const { validateConfig } = require('../../../config/constants');
...
routes.route('/register').post(validate(register, validateConfig), controller.register);
...
routes.route('/login').post(validate(login, validateConfig), controller.login);
...
routes
  .route('/is-logged-in')
  .get(validate(isLoggedIn, validateConfig), authorize(), controller.isLoggedIn);
...
routes
  .route('/refresh-token')
  .put(
    validate(refreshToken, validateConfig),
    authorize('user', true),
...
routes
  .route('/logout')
  .put(validate(refreshToken, validateConfig), authorize(), controller.logout);
...
routes
  ...
  .post(validate(forgotPassword, validateConfig), controller.forgotPassword);
...
routes
  ...
  .put(validate(resetPassword, validateConfig), controller.resetPassword);
...
routes
  ...
  .put(validate(changePassword, validateConfig), authorize(), controller.changePassword);
...
routes
  .route('/edit-profile')
  .put(validate(editProfile, validateConfig), authorize(), controller.editProfile);
...
routes
  .route('/:userName', validateConfig)
  .get(validate(getProfile), authorize(), controller.getProfile);
